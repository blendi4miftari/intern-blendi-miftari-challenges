...
const todoInprogress = document.querySelector('#inprogress-title')
const todoDone = document.querySelector('#done-title')
...
function createCard(title, description, status) {
    ...
    let cardsFromStorage = getCardsFromLocalStorage();

    cardsFromStorage.push(cardObj);

    saveCardOnLocalStorage(cardsFromStorage);

    const currentCard = createDivCard(cardObj);

    if (cardObj.status === "todo") {
        todoWrapper.appendChild(currentCard);
    } else if (cardObj.status === "inprogress") {
        inprogressWrapper.appendChild(currentCard);
    } else {
        doneWrapper.appendChild(currentCard);
    }
...
function updateCounter() {
    const cardsFromStorage = getCardsFromLocalStorage();
    const counterTodo = cardsFromStorage.filter(c => c.status === 'todo').length
    todoTitle.querySelector('span').innerText = counterTodo
    const counterInprogress = cardsFromStorage.filter(c => c.status === 'inprogress').length
    todoInprogress.querySelector('span').innerText = counterInprogress
    const counterDone = cardsFromStorage.filter(c => c.status === 'done').length
    todoDone.querySelector('span').innerText = counterDone
}
...
function createDivCard(card) {
    ...
    cardDiv.innerHTML = `
                            <div id="card-content" class="flex flex-col space-y-2">
                                <h2 class="text-lg font-medium min-h-2 min-w-0 break-words">${card.title}</h2>
                                <p class="text-gray-500 w-full min-w-0 font-normal text-base break-words min-h-4">${card.description}</p>
                            </div>
                            <div id="card-actions" class="flex flex-row justify-end items-center">
                                <button class="card-delete mt-2 flex items-center justify-center hover:bg-red-50 w-8 h-8 rounded text-slate-500 hover:text-red-600 transition-all duration-200 group">
    ...
    const editableCard = cards.find(c => c.id === card.id);
    ...
    title.addEventListener("click", () => {
        ...
        title.addEventListener("blur", e => {
            editableCard.title = title.innerText;
    ...
    description.addEventListener("click", () => {
        ...
        description.addEventListener("blur", e => {
            editableCard.description = description.innerText;
            saveCardOnLocalStorage(cards);
            description.classList.remove(
                "border",
                "border-gray-400",
...
function deleteCard(id) {
    if (confirm("Are you sure u want to delete this task?")) {
        ...
        cardsFromStorage = cardsFromStorage.filter(card => card.id !== id);
...
function renderCards() {
    ...
    const cards = cardsFromStorage.forEach(card => {
        const currentCard = createDivCard(card);
        if (card.status === "todo") {
            todoWrapper.appendChild(currentCard);
...
export { renderCards, createCard, updateCounter };
